...
(function () {
  ...
  var ROWS = [
    ...
    ["Embed DRIP back into the Drylands Program as standing infrastructure — leveraging the DSL-IP knowledge-management structure so its upkeep sits inside the Programme's mandate.",
      "Integrar de nuevo DRIP en el Programa de Tierras Secas como infraestructura permanente, aprovechando la estructura de gestión del conocimiento del DSL-IP para que su mantenimiento forme parte del mandato del Programa.",
      "Réintégrer DRIP dans le Programme des terres arides comme infrastructure permanente, en s'appuyant sur la structure de gestion des connaissances du DSL-IP afin que son entretien relève du mandat du Programme.",
      "Встроить DRIP обратно в Программу засушливых земель как постоянную инфраструктуру, опираясь на структуру управления знаниями DSL-IP, чтобы её содержание входило в мандат Программы.",
      "إعادة دمج DRIP في برنامج الأراضي الجافة بوصفها بنية تحتية دائمة — بالاستفادة من هيكل إدارة المعارف في DSL-IP بحيث تقع صيانتها ضمن تفويض البرنامج.",
      "将 DRIP 重新嵌入旱地项目，作为常设基础设施——依托 DSL-IP 的知识管理架构，使其维护纳入本项目的职责。"],

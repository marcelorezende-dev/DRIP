...
(function () {
  ...
  var ROWS = [
    // ===== nav / brand =====
    ["Dryland Research & Information Platform",
      "Plataforma de Investigación e Información sobre Tierras Secas",
      "Plateforme de recherche et d'information sur les terres arides",
      "Платформа исследований и информации по засушливым землям",
      "منصة البحوث والمعلومات الخاصة بالأراضي الجافة",
      "旱地研究与信息平台"],
    ...
    ["A funded but dormant asset, reawakened. DRIP is being activated as the Programme's dedicated knowledge platform on drylands — a single home where the research, frameworks, monitoring and stories of the Drylands Sustainable Landscapes Impact Program live, connect and stay alive.",
    ...
    ["Switch DRIP on as a dedicated drylands knowledge platform — gathering the Programme's frameworks, assessments, monitoring and stories into one navigable home, with the Constellation as its living map.",
      "Poner en marcha DRIP como plataforma de conocimiento dedicada a las tierras secas, reuniendo los marcos, evaluaciones, seguimiento e historias del Programa en un único hogar navegable, con la Constelación como su mapa vivo.",
      "Mettre en marche DRIP comme plateforme de connaissances dédiée aux terres arides, en réunissant les cadres, les évaluations, le suivi et les récits du Programme dans un lieu unique et navigable, avec la Constellation comme carte vivante.",
      "Включить DRIP как специализированную платформу знаний о засушливых землях, собрав концепции, оценки, мониторинг и истории Программы в единый удобный дом, где Созвездие служит её живой картой.",
      "تشغيل DRIP بوصفها منصة معرفة مخصّصة للأراضي الجافة — تجمع أطر البرنامج وتقييماته ورصده وقصصه في موطن واحد سهل التصفّح، مع «الكوكبة» بوصفها خريطته الحيّة.",
      "将 DRIP 启用为专门的旱地知识平台——把本项目的框架、评估、监测与故事汇聚到一个可导航的家园中，并以「星座」作为其鲜活的地图。"],
    ...
    ["The whole Programme as a living, interactive map — five lenses plus a Builder, its modules, and the finance experiment. The navigational backbone of DRIP.",
      "Todo el Programa como un mapa vivo e interactivo: cinco enfoques más un Constructor, sus módulos y el experimento de financiación. La columna vertebral de navegación de DRIP.",
      "L'ensemble du Programme sous forme de carte vivante et interactive — cinq regards, un Constructeur, ses modules et l'expérience de financement. La colonne vertébrale de navigation de DRIP.",
      "Вся Программа в виде живой интерактивной карты — пять ракурсов плюс Конструктор, её модули и финансовый эксперимент. Навигационный костяк DRIP.",
      "البرنامج بأكمله بوصفه خريطة حيّة وتفاعلية — خمس عدسات إضافةً إلى أداة بناء ووحداتها وتجربة التمويل. العمود الفقري للتنقّل في DRIP.",
      "将整个项目呈现为一张鲜活的交互地图——五个视角，外加一个构建器、其各模块与筹资实验。这是 DRIP 的导航主干。"],
    ["Open the Constellation", "Abrir la Constelación", "Ouvrir la Constellation", "Открыть Созвездие", "افتح الكوكبة", "打开星座"],
    ["Strategy · funding research", "Estrategia · investigación de financiación", "Stratégie · recherche sur le financement", "Стратегия · исследование финансирования", "الاستراتيجية · بحوث التمويل", "战略 · 筹资研究"],
    ["Innovative Funding Research for Drylands",
      "Investigación de Financiación Innovadora para Tierras Secas",
      "Recherche sur le financement innovant pour les terres arides",
      "Исследование инновационного финансирования засушливых земель",
      "بحوث التمويل المبتكر للأراضي الجافة",
      "旱地创新筹资研究"],
    ["How DRIP reaches an audience and funds itself — the multi-platform plan and the open experiment in how people allocate resources across a complex system.",
      "Cómo DRIP llega a un público y se financia: el plan multiplataforma y el experimento abierto sobre cómo las personas asignan recursos en un sistema complejo.",
      "Comment DRIP atteint un public et se finance — le plan multiplateforme et l'expérience ouverte sur la façon dont les gens allouent des ressources dans un système complexe.",
      "Как DRIP находит аудиторию и финансирует себя — мультиплатформенный план и открытый эксперимент о том, как люди распределяют ресурсы в сложной системе.",
      "كيف تصل DRIP إلى جمهورها وتموّل نفسها — الخطة متعددة المنصات والتجربة المفتوحة حول كيفية تخصيص الناس للموارد عبر نظام معقّد.",
      "DRIP 如何触达受众并实现自筹——多平台计划，以及关于人们如何在复杂系统中配置资源的开放实验。"],
    ["Read the research", "Leer la investigación", "Lire la recherche", "Читать исследование", "اقرأ البحث", "阅读研究"],
    ...
    ["The Constellation is the connective tissue — every framework, country and partner is already a node, so adding a new knowledge product to DRIP is simply lighting up where it belongs in the network.",
      "La Constelación es el tejido conectivo: cada marco, país y socio ya es un nodo, de modo que añadir un nuevo producto de conocimiento a DRIP es simplemente iluminar el lugar que le corresponde en la red.",
      "La Constellation est le tissu conjonctif — chaque cadre, pays et partenaire est déjà un nœud, de sorte qu'ajouter un nouveau produit de connaissances à DRIP revient simplement à allumer la place qui lui revient dans le réseau.",
      "Созвездие — это соединительная ткань: каждая концепция, страна и партнёр уже являются узлом, поэтому добавление нового продукта знаний в DRIP — это просто подсветка того места, где ему положено быть в сети.",
      "الكوكبة هي النسيج الرابط — فكل إطار وبلد وشريك هو عقدة بالفعل، لذا فإن إضافة منتج معرفي جديد إلى DRIP لا تعدو إضاءة الموضع الذي ينتمي إليه في الشبكة.",
      "「星座」是连接的纽带——每一个框架、国家与伙伴都已是一个节点，因此向 DRIP 添加一项新的知识产品，不过是点亮它在网络中应有的位置。"],
    ...
    // ===== 05 funding =====
    ["DRIP is already funded for activation — but durability needs a model that outlives a single grant. Alongside institutionalising it within the Programme, DRIP runs an open Innovative Funding Research for Drylands: an experiment in how people allocate resources across a complex network when they can choose their preferred lever of change.",
      "DRIP ya está financiada para su activación, pero la durabilidad requiere un modelo que perdure más allá de una sola subvención. Además de institucionalizarla dentro del Programa, DRIP lleva a cabo una Investigación abierta de Financiación Innovadora para Tierras Secas: un experimento sobre cómo las personas asignan recursos a través de una red compleja cuando pueden elegir su palanca de cambio preferida.",
      "DRIP est déjà financée pour son activation — mais la pérennité exige un modèle qui survit à une seule subvention. En plus de l'institutionnaliser au sein du Programme, DRIP mène une Recherche ouverte sur le financement innovant pour les terres arides : une expérience sur la façon dont les gens allouent des ressources à travers un réseau complexe lorsqu'ils peuvent choisir leur levier de changement préféré.",
      "DRIP уже профинансирована для активации, но устойчивость требует модели, переживающей один-единственный грант. Помимо институционализации в рамках Программы, DRIP ведёт открытое Исследование инновационного финансирования засушливых земель: эксперимент о том, как люди распределяют ресурсы по сложной сети, когда могут выбрать предпочитаемый рычаг изменений.",
      "إنّ DRIP مُموَّلة بالفعل للتفعيل — لكن الاستدامة تتطلّب نموذجًا يتجاوز عمر منحة واحدة. وإلى جانب ترسيخها مؤسسيًا داخل البرنامج، تجري DRIP بحثًا مفتوحًا في التمويل المبتكر للأراضي الجافة: تجربة حول كيفية تخصيص الناس للموارد عبر شبكة معقّدة عندما يستطيعون اختيار رافعة التغيير المفضّلة لديهم.",
      "DRIP 的启用资金早已到位——但要长久维系，需要一个能超越单笔资助的模式。在将其制度化纳入本项目的同时，DRIP 还开展一项开放的旱地创新筹资研究：一项关于人们在可自由选择偏好的变革杠杆时，如何在复杂网络中配置资源的实验。"],
    ["Institutional core", "Núcleo institucional", "Noyau institutionnel", "Институциональное ядро", "النواة المؤسسية", "制度核心"],
    ["Re-embed DRIP as standing DSL-IP infrastructure so its baseline upkeep sits inside the Programme's knowledge-management mandate — not a side project.",
      "Reintegrar DRIP como infraestructura permanente del DSL-IP para que su mantenimiento básico forme parte del mandato de gestión del conocimiento del Programa, y no de un proyecto paralelo.",
      "Réintégrer DRIP comme infrastructure permanente du DSL-IP, afin que son entretien de base relève du mandat de gestion des connaissances du Programme — et non d'un projet annexe.",
      "Снова встроить DRIP как постоянную инфраструктуру DSL-IP, чтобы её базовое содержание входило в мандат Программы по управлению знаниями, а не было побочным проектом.",
      "إعادة دمج DRIP بوصفها بنية تحتية دائمة لـ DSL-IP بحيث تقع صيانتها الأساسية ضمن تفويض إدارة المعارف في البرنامج — لا بوصفها مشروعًا جانبيًا.",
      "将 DRIP 重新嵌入为 DSL-IP 的常设基础设施，使其基础维护纳入本项目的知识管理职责，而非一个附带项目。"],
    ["The funding-research experiment", "El experimento de investigación de financiación", "L'expérience de recherche sur le financement", "Эксперимент по исследованию финансирования", "تجربة بحوث التمويل", "筹资研究实验"],
    ["An open experiment in how people allocate resources across a network when they can choose their lever of change — Patreon memberships, per-download Constellation codes and node-directed contributions.",
      "Un experimento abierto sobre cómo las personas asignan recursos a través de una red cuando pueden elegir su palanca de cambio: membresías de Patreon, códigos de la Constelación por descarga y contribuciones dirigidas a nodos.",
      "Une expérience ouverte sur la façon dont les gens allouent des ressources dans un réseau lorsqu'ils peuvent choisir leur levier de changement — adhésions Patreon, codes de la Constellation par téléchargement et contributions dirigées vers des nœuds.",
      "Открытый эксперимент о том, как люди распределяют ресурсы по сети, когда могут выбрать свой рычаг изменений — членство на Patreon, коды Созвездия за загрузку и адресные взносы на узлы.",
      "تجربة مفتوحة حول كيفية تخصيص الناس للموارد عبر شبكة عندما يستطيعون اختيار رافعة التغيير الخاصة بهم — عضويات Patreon، ورموز الكوكبة لكل تنزيل، والمساهمات الموجَّهة إلى العُقد.",
      "一项开放实验：当人们可以选择自己的变革杠杆时，如何在网络中配置资源——Patreon 会员、按次下载的星座代码，以及指向特定节点的捐助。"],
    ["See the research →", "Ver la investigación →", "Voir la recherche →", "Смотреть исследование →", "اطّلع على البحث →", "查看研究 →"],
    ["Kept honest", "Con integridad", "En toute honnêteté", "Под контролем честности", "تحت الرقابة والنزاهة", "保持诚信"],
    ["Any contribution mechanic is a transparent, experimental and symbolic allocation — not a transfer to a named beneficiary — and is reconciled against FAO and platform rules before launch.",
      "Cualquier mecánica de contribución es una asignación transparente, experimental y simbólica —no una transferencia a un beneficiario concreto— y se concilia con las normas de la FAO y de la plataforma antes de su lanzamiento.",
      "Tout mécanisme de contribution est une allocation transparente, expérimentale et symbolique — et non un transfert vers un bénéficiaire nommé — et il est mis en conformité avec les règles de la FAO et de la plateforme avant son lancement.",
      "Любой механизм взносов — это прозрачное, экспериментальное и символическое распределение, а не перевод конкретному получателю, и он согласуется с правилами ФАО и платформы до запуска.",
      "أي آلية مساهمة هي تخصيص شفّاف وتجريبي ورمزي — وليست تحويلاً إلى مستفيد مُسمّى — وتجري مطابقتها مع قواعد الفاو والمنصة قبل الإطلاق.",
      "任何捐助机制都是透明、实验性且象征性的配置——并非向具名受益人转账——并在上线前依照粮农组织与平台规则进行核对。"],
    ...
    ["Funding Research", "Investigación de financiación", "Recherche sur le financement", "Исследование финансирования", "بحوث التمويل", "筹资研究"],
    ["Strategy & the experiment", "Estrategia y el experimento", "Stratégie et expérience", "Стратегия и эксперимент", "الاستراتيجية والتجربة", "战略与实验"],
    ["Support the experiment", "Apoye el experimento", "Soutenir l'expérience", "Поддержать эксперимент", "ادعم التجربة", "支持这项实验"],
    ["Back DRIP on Patreon — pick a node and take part in the innovative-finance experiment.",
      "Apoye DRIP en Patreon: elija un nodo y participe en el experimento de financiación innovadora.",
      "Soutenez DRIP sur Patreon — choisissez un nœud et participez à l'expérience de financement innovant.",
      "Поддержите DRIP на Patreon — выберите узел и примите участие в эксперименте инновационного финансирования.",
      "ادعم DRIP عبر Patreon — اختر عقدة وشارك في تجربة التمويل المبتكر.",
      "在 Patreon 上支持 DRIP——选择一个节点，参与创新融资实验。"],
    ...
    ["DRIP · Dryland Research & Information Platform — hosted under the DSL-IP Knowledge Management structure. A project of the DSL-IP, actively overseen by a UN Officer (programme design · complex programme management · AI · rural development); related to the Programme's behaviour-change research. Pending institutional sign-off.",
      "DRIP · Plataforma de Investigación e Información sobre Tierras Secas — alojada bajo la estructura de Gestión del Conocimiento del DSL-IP. Un proyecto del DSL-IP, supervisado activamente por un funcionario de la ONU (diseño de programas · gestión de programas complejos · IA · desarrollo rural); relacionado con la investigación del Programa sobre cambio de comportamiento. A la espera de la aprobación institucional.",
      "DRIP · Plateforme de recherche et d'information sur les terres arides — hébergée au sein de la structure de gestion des connaissances du DSL-IP. Un projet du DSL-IP, activement supervisé par un fonctionnaire de l'ONU (conception de programmes · gestion de programmes complexes · IA · développement rural) ; lié à la recherche du Programme sur le changement de comportement. En attente de validation institutionnelle.",
      "DRIP · Платформа исследований и информации по засушливым землям — размещена в рамках структуры управления знаниями DSL-IP. Проект DSL-IP, под активным надзором сотрудника ООН (проектирование программ · управление сложными программами · ИИ · сельское развитие); связан с исследованием Программы по изменению поведения. Ожидает институционального утверждения.",
      "DRIP · منصة البحوث والمعلومات الخاصة بالأراضي الجافة — مستضافة ضمن هيكل إدارة المعارف في DSL-IP. مشروع تابع لـ DSL-IP، يشرف عليه بنشاط موظف في الأمم المتحدة (تصميم البرامج · إدارة البرامج المعقّدة · الذكاء الاصطناعي · التنمية الريفية)؛ ويتّصل ببحوث البرنامج المتعلّقة بتغيير السلوك. في انتظار الموافقة المؤسسية.",
      "DRIP · 旱地研究与信息平台——托管于 DSL-IP 知识管理架构之下。DSL-IP 的一个项目，由一名联合国官员积极监督（项目设计 · 复杂项目管理 · 人工智能 · 农村发展）；与本项目的行为改变研究相关。尚待机构正式批准。"],

    // ===== constellation modal =====
    ["Before you enter", "Antes de entrar", "Avant d'entrer", "Прежде чем войти", "قبل أن تدخل", "进入之前"],
    ["The Constellation is an experiment", "La Constelación es un experimento", "La Constellation est une expérience", "Созвездие — это эксперимент", "الكوكبة تجربة", "「星座」是一项实验"],
